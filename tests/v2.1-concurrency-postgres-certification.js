...
async function setupDatabase(db) {
  ...
  await db.query('INSERT INTO workspaces(id,name) VALUES ($1,$2)', ['00000000-0000-0000-0000-000000000032', 'concurrency-cert']);
...
async function setup(db) {
  ...
  await db.query(`INSERT INTO v2_1.productions(id, workspace_id, name, status, metadata) VALUES ($1,$2,$3,'DRAFT','{}')`, [productionId, workspaceId, `concurrency-cert-${productionId}`]);
  await db.query(`INSERT INTO v2_1.jobs(id, production_id, stage, idempotency_key, status) VALUES ($1,$2,'SIGNAL',$3,'QUEUED')`, [jobId, productionId, `concurrency-job-${jobId}`]);
  return { jobId };
}
...
async function raceClaims(jobId) {
  ...
  try {
    return await Promise.all(clients.map(async (c, i) => {
      ...
      const result = await c.query('SELECT * FROM v2_1.claim_job_for_production($1,$2,$3,$4)', [jobId, productionId, `cert-worker-${i}`, 30]);
...
async function main() {
  ...
  try {
    ...
    if (successfulClaims.length !== 1) throw new Error(`Concurrency certification failed: expected exactly 1 successful job claim, got ${successfulClaims.length}`);
    const owner = successfulClaims[0].worker_id;
    const job = await db.query('SELECT status, worker_id FROM v2_1.jobs WHERE id=$1', [jobId]);
    if (job.rows[0].status !== 'RUNNING' || job.rows[0].worker_id !== owner) throw new Error('Concurrency certification failed: persisted job ownership is inconsistent');

    const stageClaims = [];
    const stageClients = Array.from({ length: WORKERS }, () => client());
    await Promise.all(stageClients.map((c) => c.connect()));
    try {
      await Promise.all(stageClients.map((c, i) => c.query('SELECT * FROM v2_1.claim_stage($1,$2,$3)', [jobId, owner, 30]).then((r) => stageClaims.push(...r.rows.map((row) => ({ ...row, contender: i }))))));
    } finally { await Promise.all(stageClients.map((c) => c.end())); }
    if (stageClaims.length !== 1) throw new Error(`Concurrency certification failed: expected exactly 1 successful stage claim, got ${stageClaims.length}`);
