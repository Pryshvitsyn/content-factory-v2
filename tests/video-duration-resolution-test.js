...
async function main() {
  ...
  const original={title:'Synthetic timing test',objective:'Show craft',targetPlatform:'Reels',targetDurationSeconds:8,
    hook:'Opening',coreMessage:'Craft',cta:'Visit',creativeConcept:'Synthetic room transformation',visualStyle:'Realistic craft',storyboard:[3.75,4.25].map((durationSeconds,index)=>({shotId:`s${index+1}`,assetId:`a${index+1}`,
  ...
  assert.deepEqual(video.shotCapabilities.map(s=>s.resolvedSettings.providerDurationSeconds),[4,5]);
  ...
  assert.deepEqual(assets.map(a=>a.generation_requirements.target_clip_duration_ms),[3750,4250]);
  assert.deepEqual(assets.map(a=>a.generation_requirements.resolved_settings.providerDurationSeconds),[4,5]);
  ...
  assert.equal(bounded.providerExecutions,0);assert.equal(bounded.plan.editorialDurationSeconds,3.75);assert.equal(bounded.plan.providerDurationSeconds,4);
  ...
  const args=buildFfmpegArgs({assembly:{durationMs:8000,clips:[{kind:'video',durationMs:3750},{kind:'video',durationMs:4250}]},inputPaths:['synthetic-a.mp4','synthetic-b.mp4'],outputPath:'synthetic-master.mp4'});
  assert(args.join(' ').includes('trim=start=0.000:duration=3.750'));assert(args.join(' ').includes('trim=start=0.000:duration=4.250'));
  const probe={size:10,videoCodec:'h264',durationMs:4000};assert.equal(validateMediaProbe({kind:'video',probe,expectedDurationMs:3750}).status,'PASS');
  assert.throws(()=>validateMediaProbe({kind:'video',probe:{...probe,durationMs:1000},expectedDurationMs:3750}),e=>e.code==='MEDIA_DURATION_TOO_SHORT');
