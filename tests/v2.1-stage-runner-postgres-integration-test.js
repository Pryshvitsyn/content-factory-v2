...
async function run() {
  ...
  try {
    ...
    await db.query(`
      ...
      FROM v2_1.productions p WHERE p.idempotency_key='stage-runner-production'
      ON CONFLICT (production_id, idempotency_key) DO NOTHING;
    `);

    const job = (await db.query(`
      SELECT j.id FROM v2_1.jobs j JOIN v2_1.productions p ON p.id=j.production_id
      WHERE p.idempotency_key='stage-runner-production' AND j.idempotency_key='stage-runner-job'
    `)).rows[0];
    assert.ok(job);
    ...
    const runner = new StageRunner({
      execution, providerGateway: gateway, artifactService: artifacts,
      handlers: {
        SIGNAL: async ({ providerGateway }) => {
          const result = await providerGateway.generate({ capability: 'text-generation', prompt: 'signal from postgres job' });
          return {
            artifacts: [{ artifactId: 'postgres-stage-runner-artifact', type: 'text', content: result.output, provider: result.provider, model: result.model }],
    ...
    const persisted = (await db.query(`
      SELECT status, worker_id, output_artifacts, output_fingerprint
    ...
    const bytes = await storage.get({ key: result.outputArtifacts[0] });
    assert.equal(bytes.toString('utf8'), 'generated:signal from postgres job');
