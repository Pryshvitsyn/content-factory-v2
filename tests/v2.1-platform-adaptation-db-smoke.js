...
async function main() {
  ...
  try {
    ...
    for (const [stage, output] of Object.entries({ SIGNAL:'SIGNAL_SET', IDEA:'IDEA_SET', BRIEF:'CONTENT_BRIEF', CONCEPT:'CONCEPT', SCRIPT:'SCRIPT', BIBLE:'PRODUCTION_BIBLE', SHOT_PLAN:'SHOTS', ASSET_PLAN:'ASSET_REQUIREMENTS', ASSET_GENERATION:'ASSETS', CONTINUITY:'CONTINUITY_REPORT' })) {
    ...
    const editManifest = {
      ...
      sourceArtifacts: { continuityReportArtifactId: 'continuity-id' },
      timeline: [{ index: 1, shotId: 'shot-1', startMs: 0, endMs: 2000, durationMs: 2000, assetVersionIds: ['asset-version-1'] }],
      durationMs: 2000,
      renderPolicy: { mode: 'PROVIDER_NEUTRAL' },
    };
    const editHash = fingerprint(editManifest);
    await client.query(`INSERT INTO v2_1.artifact_versions(artifact_id,version,input_hash,output_hash,metadata) VALUES($1,1,'continuity-hash',$2,$3::jsonb)`, [editArtifact.rows[0].id, editHash, JSON.stringify({ stage:'EDIT', contextFingerprint:`ctx-${suffix}`, continuityArtifactId:'continuity-id', continuityFingerprint:'continuity-hash', durationMs:2000, manifest:editManifest })]);
    ...
    const reusedStage = await client.query(`SELECT id FROM v2_1.stage_runs WHERE job_id=$1 AND stage='PLATFORM_ADAPTATION' AND attempt=2`, [jobId]);
    if (reusedStage.rowCount) throw new Error('Unexpected retry stage created after successful PLATFORM_ADAPTATION');
