...
const { FilesystemStorage } = require('../src/storage/filesystem-storage');
...
async function main() {
  ...
  try {
    assert.throws(() => new FilesystemStorage('relative-root'), /absolute path/);
    ...
    await assert.rejects(storage.put(key, Buffer.from('different')), /already exists/);
    await assert.rejects(storage.put('../escape.txt', payload), /path traversal/);
    await assert.rejects(storage.put('/absolute.txt', payload), /relative POSIX path/);
    await assert.rejects(storage.put('artifacts\\escape.txt', payload), /relative POSIX path/);
