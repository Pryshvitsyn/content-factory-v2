...
async function main() {
  ...
  try {
    ...
    const version = await client.query(`INSERT INTO v2_1.asset_versions(asset_id,version,data,source_artifact_id) VALUES($1,1,'{"appearance":"stable"}'::jsonb,$2) RETURNING id`, [asset.rows[0].id, script.rows[0].id]);
