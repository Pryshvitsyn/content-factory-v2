...
const migrations = ['migrations/002_v2_1_execution.sql','migrations/20260819_v2_1_stage_input_propagation.sql'];
function makeClient() { return new Client({ host: process.env.PGHOST || '127.0.0.1', port: Number(process.env.PGPORT || 5432), user: process.env.PGUSER || 'postgres', password: process.env.PGPASSWORD || 'postgres', database: process.env.PGDATABASE || 'content_os' }); }
...
async function run() {
  ...
  const fakeNvidia = {
    ...
    async generate({ prompt, model }) { return assertProviderResult({ provider: 'nvidia', model: model || this.model, output: `generated:${prompt}`, requestId: `postgres-multi-${Date.now()}`, usage: { total_tokens: 4 } }); },
  };
  ...
  try {
    ...
    await db.query(`INSERT INTO workspaces(id, name) VALUES ('00000000-0000-0000-0000-000000000021', 'multi-stage-postgres-cert') ON CONFLICT (id) DO NOTHING`);
    await db.query(`INSERT INTO v2_1.productions(workspace_id, name, status, metadata) VALUES ('00000000-0000-0000-0000-000000000021', $1, 'DRAFT', '{"test":true}') ON CONFLICT (workspace_id, name) DO NOTHING`, [productionKey]);
    await db.query(`INSERT INTO v2_1.jobs(production_id, stage, status, max_attempts, idempotency_key) SELECT p.id, 'SIGNAL', 'QUEUED', 3, $2 FROM v2_1.productions p WHERE p.name=$1 ON CONFLICT (production_id, idempotency_key) DO NOTHING`, [productionKey, jobKey]);
    const job = (await db.query(`SELECT j.id, j.production_id FROM v2_1.jobs j JOIN v2_1.productions p ON p.id=j.production_id WHERE p.name=$1 AND j.idempotency_key=$2`, [productionKey, jobKey])).rows[0];
    assert.ok(job);
    const claimedJob = await execution.claimJobForProduction(db, { jobId: job.id, productionId: job.production_id, workerId, leaseSeconds: 60 });
    ...
    const handlers = Object.fromEntries(STAGE_ORDER.map((stage) => [stage, async ({ providerGateway, inputArtifacts }) => {
      ...
      const result = await providerGateway.generate({ capability: 'text_generation', prompt: `${stage}|input=${input}` });
      return { artifacts: [{ artifactId: `pg-multi-${stage.toLowerCase()}`, type: 'text', content: result.output, provider: result.provider, model: result.model }], provenance: result.provenance };
    }]));
    ...
    for (let index = 0; index < STAGE_ORDER.length; index += 1) {
      const stageRun = await execution.claimNextStage(db, { jobId: job.id, workerId, leaseSeconds: 60 });
      ...
      const persisted = (await db.query('SELECT status, input_artifacts, output_artifacts, input_fingerprint, output_fingerprint, worker_id FROM v2_1.stage_runs WHERE id=$1', [stageRun.id])).rows[0];
    ...
    assert.equal((await db.query(`SELECT count(*)::int AS count FROM v2_1.stage_runs WHERE job_id=$1 AND status='COMPLETED'`, [job.id])).rows[0].count, STAGE_ORDER.length);
    await execution.completeJob(db, { jobId: job.id, workerId });
    const final = (await db.query(`SELECT j.status AS job_status, p.status AS production_status FROM v2_1.jobs j JOIN v2_1.productions p ON p.id=j.production_id WHERE j.id=$1`, [job.id])).rows[0];
