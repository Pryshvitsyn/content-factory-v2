...
async function main() {
  ...
  const plan = createGenerationPlan({ reference, motionSpec: { reference, units: [unit] }, providerSelection: selection, durationSeconds: 5, seed: 101 });
  assert.equal(plan.request.capability, 'IMAGE_TO_VIDEO'); assert.ok(plan.negativePrompt.includes('no white ring')); assert.equal(plan.expectedPaidCalls, 1);
