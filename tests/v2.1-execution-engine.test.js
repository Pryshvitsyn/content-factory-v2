...
const EXPECTED = [
  'SIGNAL','IDEA','BRIEF','CONCEPT','SCRIPT','BIBLE','ASSET_PLAN','SHOT_PLAN',
  'ASSET_GENERATION','CONTINUITY','EDIT','PLATFORM_ADAPTATION','VALIDATION',
...
test('stable fingerprints ignore object key order', () => {
  ...
  assert.equal(
    fingerprint({ request: { b: 2, a: 1 } }),
    fingerprint({ request: { a: 1, b: 2 } })
  );
});

test('stage fingerprints change when execution inputs change', () => {
  assert.notEqual(
    fingerprint({ stage: 'SCRIPT', input: { concept: 'A' } }),
    fingerprint({ stage: 'SCRIPT', input: { concept: 'B' } })
  );
});
