...
test('request order changes fingerprint and schema comparison reports drift without mutation',()=>{const one=resolveVideoModelRequest({provider:'replicate',model:SEEDANCE_25.model,request:{...base,resolvedInputMode:INPUT_MODES.MULTIMODAL_REFERENCE,referenceImages:['a','b']}}),two=resolveVideoModelRequest({provider:'replicate',model:SEEDANCE_25.model,request:{...base,resolvedInputMode:INPUT_MODES.MULTIMODAL_REFERENCE,referenceImages:['b','a']}});assert.notEqual(one.requestFingerprint,two.requestFingerprint);const properties=Object.fromEntries(SEEDANCE_25.providerFields.map(x=>[x,{}]));assert.equal(compareVideoModelSchema({provider:'replicate',model:SEEDANCE_25.model,schema:{properties}}).status,'CURRENT');properties.new_field={};assert.deepEqual(compareVideoModelSchema({provider:'replicate',model:SEEDANCE_25.model,schema:{properties}}).added,['new_field']);});
