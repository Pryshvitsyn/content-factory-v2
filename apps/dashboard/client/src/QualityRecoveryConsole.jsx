...
export function QualityRecoveryConsole() {
  ...
  async function recover(item) {
    ...
    try {
      ...
      if (plan.action === 'REGENERATE_SHOT') {
        ...
        const recoveryReason = continuity ? null : 'SOURCE_GEOMETRY';
        ...
        if (!approved) { setMessage(`${continuity ? 'Continuity' : 'Geometry'} recovery cancelled before any provider call.`); return; }
  ...
  return <aside className="quality-recovery-console" aria-label="Quality evidence recovery">
    <div className="quality-recovery-console__head"><div><span>V2.10.3 · SAFE RECOVERY</span><strong>Source recovery & paid asset protection</strong></div><span className="quality-recovery-console__zero">BOUNDED · EXPLICIT</span></div>
    <p>Evidence-only issues reuse immutable media. Objective geometry and cross-shot continuity failures require an explicitly confirmed failed-shot replacement; rejected versions and good assets stay immutable.</p>
