    ...
    <div className="quality-recovery-console__list">{items.map((item) => {
      ...
      return <article key={item.id} className="quality-recovery-console__item">
        ...
        {recovery.eligible && recovery.recoveryKind === 'SOURCE_CREATIVE' ? <div className="quality-recovery-console__creative-context">
          <strong>Evaluator evidence · audit only</strong>
          <p>{recovery.failureReason || 'No evaluator reason was recorded.'}</p>
          <strong>Sanitized provider recovery context</strong>
          <p>{recovery.sanitizedRecoveryObservation || 'Visible content contradicted the approved shot state.'}</p>
          <label htmlFor={`creative-recovery-${item.id}`}>Explicit operator corrective instruction <small>trusted input · optional · editable before paid confirmation</small></label>
