...
export function ProductionDetail({ production, onBack = () => {} }) {
  ...
  useEffect(() => { if (!state.data || TERMINAL.has(state.data.item.operationalStatus)) return undefined; const timer = setInterval(() => setRevision((value) => value + 1), 5000); return () => clearInterval(timer); }, [state.data?.item?.operationalStatus]);
