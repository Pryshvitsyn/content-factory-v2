...
export function ProductionDetail({ production, onBack = () => {} }) {
  ...
  async function retrySemantic(item) {
    ...
    try {
      ...
      const approved = window.confirm(`Retry semantic evaluation using the existing immutable source?\n\nSEMANTIC RETRY STEP\nVideo generation: 0\nSpeech generation: 0\nSemantic evaluation: ${plan.expectedSemanticEvaluations}\n\nPOSSIBLE POST-PASS CONTINUATION\nExisting video reused: ${plan.existingSourceVideo ? 'YES' : 'NO'}\nExisting voice reused: ${plan.existingVoiceReused ? 'YES' : 'NO'}\nPossible speech generation after semantic PASS: ${plan.possiblePostPassSpeechGenerations}\nVideo regeneration: 0`);
  ...
  return <State state={state}>{({ item, stages, artifacts }) => <><button className="back" onClick={onBack}>← All productions</button><div className="detail-head"><div><span className="eyebrow">{item.brandName}</span><h2>{item.title || item.name}</h2><code>{item.id}</code></div><Badge value={item.operationalStatus} /></div><p className="page-note">{item.renderMode} · {item.renderer} · Publication: NOT TRIGGERED</p>{message ? <div className="notice">{message}</div> : null}
    ...
    {Object.keys(item.jobError || {}).length ? <Section title="Errors / recovery"><div className="error-panel"><strong>{item.jobError.code || 'Production failed'}</strong><p>{item.jobError.message || 'See diagnostics.'}</p></div>{item.semanticRetry?.eligible ? <div className="notice"><strong>Semantic-only recovery</strong><p>Existing source video: {item.semanticRetry.media?.existingSourceVideo ? 'REUSED' : 'MISSING'}<br/>Existing voice: {item.semanticRetry.media?.reusedSpeechAssets > 0 ? 'REUSED' : 'MISSING'}<br/>Semantic evaluation: 1<br/>Possible post-pass speech generation: {item.semanticRetry.media?.possiblePostPassSpeechGenerations ?? 0}<br/>Video regeneration: 0</p><button className="secondary" disabled={action || item.semanticRetry.media?.existingSourceVideo === false} onClick={() => retrySemantic(item)}>RETRY SEMANTIC EVALUATION</button></div> : null}</Section> : null}<div className="actions detail-actions">{item.status === 'DRAFT' && item.jobStatus === 'QUEUED' ? <button className="start" disabled={action} title="Explicitly start this prepared production" onClick={() => start(item)}>START PRODUCTION</button> : null}<button className="secondary" disabled={action || item.jobStatus !== 'RETRYING'} title="Retry continues the same technical execution" onClick={() => retry(item)}>RETRY SAME EXECUTION</button><button className="regenerate" disabled={action || ['RUNNING','QUEUED'].includes(item.jobStatus) || item.ambiguousExecutions > 0} title="Regenerate creates a new immutable production" onClick={() => regenerate(item)}>REGENERATE NEW REVISION</button></div>
  </>}</State>;
}
...
function ShotInspector({ shot, evaluation, continuity, disabled, regenerate }) {
  ...
  return <article className="shot-inspector"><div className="detail-head"><div><strong>{shot.shotId} · {shot.purpose}</strong><small>{shot.durationSeconds}s · {shot.assetId}</small></div><Badge value={evaluation?.status || 'NOT_EVALUATED'} /></div>
    <div className="plan-grid"><KeyValue label="Provider / model" value={[evaluation?.provider, evaluation?.model].filter(Boolean).join(' · ')} /><KeyValue label="Profile" value={profileLabel(evaluation?.profile)} /><KeyValue label="Source" value={probe.width ? `${probe.width}×${probe.height} · ${probe.fps}fps · ${probe.videoCodec}` : 'Not generated'} /><KeyValue label="Seed" value={evaluation?.seed ?? shot.seed} /><KeyValue label="Visual score" value={evaluation?.score ?? 'n/a'} /><KeyValue label="Deterministic" value={evaluation?.deterministicVisual?.status || 'NOT_EVALUATED'} /><KeyValue label="Temporal" value={evaluation?.temporal?.status || 'NOT_EVALUATED'} /><KeyValue label="Semantic" value={evaluation?.semantic?.status || 'NOT_EVALUATED'} /><KeyValue label="Semantic evaluator" value={[evaluation?.semantic?.metadata?.provider, evaluation?.semantic?.metadata?.model].filter(Boolean).join(' · ')} /><KeyValue label="Continuity" value={continuity?.status || 'NOT_EVALUATED'} /></div>
