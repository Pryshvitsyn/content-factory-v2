...
export function ProductionDetail({ production, onBack = () => {} }) {
  ...
  return <State state={state}>{({ item, stages, artifacts }) => <><button className="back" onClick={onBack}>← All productions</button><div className="detail-head"><div><span className="eyebrow">{item.brandName}</span><h2>{item.title || item.name}</h2><code>{item.id}</code></div><Badge value={item.operationalStatus} /></div><p className="page-note">{item.renderMode} · {item.renderer} · Publication: NOT TRIGGERED</p>{message ? <div className="notice">{message}</div> : null}
    ...
    {Object.keys(item.jobError || {}).length ? <Section title="Errors / recovery"><div className="error-panel"><strong>{item.jobError.code || 'Production failed'}</strong><p>{item.jobError.message || 'See diagnostics.'}</p></div>{item.semanticRetry?.eligible ? <div className="notice"><strong>Semantic-only recovery</strong><p>Video generation: 0<br/>Speech generation: 0<br/>Semantic evaluation: 1</p><button className="secondary" disabled={action || item.ambiguousExecutions > 0} onClick={() => retrySemantic(item)}>RETRY SEMANTIC EVALUATION</button></div> : null}</Section> : null}<div className="actions detail-actions">{item.status === 'DRAFT' && item.jobStatus === 'QUEUED' ? <button className="start" disabled={action} title="Explicitly start this prepared production" onClick={() => start(item)}>START PRODUCTION</button> : null}<button className="secondary" disabled={action || item.jobStatus !== 'RETRYING'} title="Retry continues the same technical execution" onClick={() => retry(item)}>RETRY SAME EXECUTION</button><button className="regenerate" disabled={action || ['RUNNING','QUEUED'].includes(item.jobStatus) || item.ambiguousExecutions > 0} title="Regenerate creates a new immutable production" onClick={() => regenerate(item)}>REGENERATE NEW REVISION</button></div>
  </>}</State>;
}
