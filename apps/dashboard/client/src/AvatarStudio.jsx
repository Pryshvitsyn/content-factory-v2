...
function CreateAvatar({ brands, onCreated }) {
  ...
  const [avatar, setAvatar] = useState(null); const [source, setSource] = useState(null); const [passport, setPassport] = useState(null); const [passports, setPassports] = useState([]);
  ...
  async function registerPassport() {
    ...
    }) }); const candidate = { ...result.passport, panels: { frontal: form.frontal, threeQuarter: form.threeQuarter, profile: form.profile } };
      setPassports((current) => [...current, candidate]); setPassport(candidate); setAvatar(result.avatar); setStep(5); } catch (cause) { setError(cause); } finally { setBusy(false); }
  }
  ...
  return <section className="avatar-wizard"><div className="wizard-steps">{steps.map((name, index) => <span className={step === index ? 'active' : index < step ? 'done' : ''} key={name}>{index + 1} · {name}</span>)}</div><ErrorPanel error={error} />
    ...
    {step === 5 ? <div className="panel avatar-form"><h2>Human approval</h2><p>Compare registered candidates and explicitly select the one passport to certify.</p><div className="passport-candidates">{passports.map((candidate) => <label className={passport?.id === candidate.id ? 'selected' : ''} key={candidate.id}><input type="radio" name="passport-candidate" checked={passport?.id === candidate.id} onChange={() => setPassport(candidate)} /><strong>Candidate {candidate.candidateNo}</strong><span>{candidate.panels.frontal}</span><span>{candidate.panels.threeQuarter}</span><span>{candidate.panels.profile}</span></label>)}</div><div className="warning-panel">Certification is an explicit immutable human decision. It does not generate or publish content.</div><div className="passport-actions"><button type="button" className="secondary" onClick={() => { setForm((current) => ({ ...current, frontal: '', threeQuarter: '', profile: '' })); setStep(4); }}>REGISTER ANOTHER CANDIDATE</button><button className="primary" disabled={busy || !passport} onClick={certify}>CERTIFY SELECTED PASSPORT · LEVEL UP TO L1</button></div></div> : null}
