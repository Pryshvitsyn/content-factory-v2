...
function buildIdeaRequest({ production, context, signal }) {
  return {
    ...
    context,
    input: { signal: signal || {} },
    outputContract: { type: 'IDEA_SET', required: ['ideas'], ideas: 'array of 3-5 distinct content ideas; each idea has id, title, premise, hook, angle, rationale' },
  };
}

function buildMessages(request) {
  return [
    { role: 'system', content: 'You are the IDEA stage of a production content factory. Creative truth is authoritative: tenant, business, brand, audience, offering, strategy and universe rules must be respected. Return JSON only. Do not mention providers, models, APIs or implementation details. Do not invent claims that conflict with compliance rules. Create 3-5 genuinely different ideas, not cosmetic rewrites.' },
...
async function executeIdeaStage({ client, productionId, stageRunId, workerId, signal = {}, provider = 'nvidia', providerCall = null } = {}) {
  ...
  if (stage.status !== 'RUNNING' || stage.worker_id !== workerId) throw new Error('IDEA stage lease is not owned by this worker');

  const request = buildIdeaRequest({ production, context: production.context_snapshot, signal });
  ...
  try {
    ...
    const artifact = await client.query(`INSERT INTO v2_1.artifacts(artifact_type, production_id, status) VALUES ('IDEA_SET', $1, 'VALID') RETURNING id`, [productionId]);
    const artifactId = artifact.rows[0].id;
    await client.query(`INSERT INTO v2_1.artifact_versions (artifact_id, version, provider_id, model_id, input_hash, output_hash, metadata) VALUES ($1,1,$2,$3,$4,$5,$6::jsonb)`, [artifactId, providerId, modelId, requestHash, outputHash, JSON.stringify({ capability: 'TEXT_GENERATION', generationRunId })]);
...
module.exports = { stableStringify, fingerprint, buildIdeaRequest, validateIdeaSet, callNvidia, executeIdeaStage };
