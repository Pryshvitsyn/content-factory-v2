...
function buildBriefRequest({ production, context, idea, signal }) {
  return {
    capability: 'TEXT_GENERATION',
    production: { id: production.id, request: production.request_snapshot || {}, contextFingerprint: production.context_fingerprint },
    context,
    sources: { ideaArtifactId: idea.id, ideaArtifactVersion: idea.version, ideaOutputHash: idea.outputHash, idea: idea.value },
    input: { signal: signal || {} },
    outputContract: {
      type: 'CONTENT_BRIEF',
      required: ['objective', 'audience', 'promise', 'keyMessage', 'cta', 'creativeDirection', 'constraints'],
    },
  };
}
...
function buildMessages(request) {
  return [
    { role: 'system', content: 'You are the BRIEF stage of a production content factory. Creative truth is authoritative: tenant, business, brand, audience, offering, strategy, universe and canonical IDEA must be respected. Convert the selected IDEA into a precise production brief. Return JSON only. Do not mention providers, models, APIs or implementation details. Do not invent claims that conflict with compliance rules.' },
...
async function callNvidia({ request, client, model }) {
  ...
  const response = await actualClient.chat.completions.create({ model: actualModel, messages: buildMessages(request), temperature: 0.4, max_tokens: 2500 });
  return { parsed: extractJson(response.choices?.[0]?.message?.content), raw: response };
}

function validateBrief(value) {
  if (!value || typeof value !== 'object' || Array.isArray(value)) throw new Error('CONTENT_BRIEF must be an object');
  const fields = ['objective', 'audience', 'promise', 'keyMessage', 'cta', 'creativeDirection'];
  for (const field of fields) {
    if (typeof value[field] !== 'string' || !value[field].trim()) throw new Error(`CONTENT_BRIEF is missing ${field}`);
  }
  if (!value.constraints || typeof value.constraints !== 'object' || Array.isArray(value.constraints)) throw new Error('CONTENT_BRIEF is missing constraints');
  return true;
}

async function loadIdeaArtifact(client, productionId) {
  const result = await client.query(
    `SELECT gr.id AS generation_run_id, gr.artifact_id, gr.response, av.version, av.output_hash
      ...
      WHERE p.id = $1 AND sr.stage = 'IDEA' AND gr.status = 'COMPLETED' AND gr.artifact_id IS NOT NULL
      ORDER BY av.version DESC, gr.completed_at DESC LIMIT 1`, [productionId]);
  const row = result.rows[0];
  if (!row) throw new Error('Completed IDEA artifact is required before BRIEF generation');
...
async function executeBriefStage({ client, productionId, stageRunId, workerId, signal = {}, provider = 'nvidia', providerCall = null } = {}) {
  ...
  if (!stage || stage.stage !== 'BRIEF') throw new Error('Stage run is not a BRIEF stage for this production');
  if (stage.status !== 'RUNNING' || stage.worker_id !== workerId) throw new Error('BRIEF stage lease is not owned by this worker');

  const idea = await loadIdeaArtifact(client, productionId);
  const request = buildBriefRequest({ production, context: production.context_snapshot, idea, signal });
  ...
  if (existing.rowCount) {
    ...
    if (row.status === 'RUNNING') throw new Error('Identical BRIEF generation is already running');
  }
  ...
  if (!generation.rowCount) throw new Error('Identical BRIEF generation was claimed by another worker');
  ...
  try {
    ...
    validateBrief(result.parsed);
    const outputHash = fingerprint(result.parsed);
    const artifact = await client.query(`INSERT INTO v2_1.artifacts(artifact_type, production_id, status) VALUES ('CONTENT_BRIEF', $1, 'VALID') RETURNING id`, [productionId]);
    const artifactId = artifact.rows[0].id;
    await client.query(`INSERT INTO v2_1.artifact_versions(artifact_id, version, provider_id, model_id, input_hash, output_hash, metadata) VALUES ($1,1,$2,$3,$4,$5,$6::jsonb)`, [artifactId, providerId, modelId, requestHash, outputHash, JSON.stringify({ capability: 'TEXT_GENERATION', generationRunId, sourceArtifactId: idea.id, sourceArtifactVersion: idea.version, sourceOutputHash: idea.outputHash })]);
...
module.exports = { stableStringify, fingerprint, buildBriefRequest, validateBrief, callNvidia, executeBriefStage };