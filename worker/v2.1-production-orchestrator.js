...
class ProductionOrchestrator {
  ...
  async createProduction({ client, workspaceId, name, request, workerId, targetStage = 'SCRIPT' } = {}) {
    ...
    const productionResult = await client.query(`INSERT INTO v2_1.productions(workspace_id, name, status, metadata) VALUES ($1,$2,'DRAFT',$3::jsonb) RETURNING *`, [workspaceId, name, JSON.stringify({ request })]);
    const production = productionResult.rows[0];
    const jobResult = await client.query(`INSERT INTO v2_1.jobs(production_id, stage, idempotency_key, payload) VALUES ($1,'SIGNAL',$2,$3::jsonb) RETURNING *`, [production.id, `production:${production.id}:planning`, JSON.stringify({ request })]);
