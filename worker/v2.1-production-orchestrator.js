...
const { executeValidationStage } = require('./v2.1-validation');
...
const VALIDATION_VERTICAL_SLICE = Object.freeze([...PLATFORM_ADAPTATION_VERTICAL_SLICE, 'VALIDATION']);

function requireClient(client) { if (!client || typeof client.query !== 'function') throw new Error('client is required'); }
function requireWorker(workerId) { if (typeof workerId !== 'string' || !workerId.trim()) throw new Error('workerId is required'); }
function normalizeSignal(signal) { return signal && typeof signal === 'object' && !Array.isArray(signal) ? signal : {}; }
...
const HANDLERS = Object.freeze({ IDEA: executeIdeaStage, BRIEF: executeBriefStage, CONCEPT: executeConceptStage, SCRIPT: executeScriptStage, BIBLE: executeBibleStage, SHOT_PLAN: executeShotPlanStage, ASSET_PLAN: executeAssetPlanStage, EDIT: executeEditStage, PLATFORM_ADAPTATION: executePlatformAdaptationStage, VALIDATION: executeValidationStage });
...
async function executeClaimedStage(client, { productionId, stage, stageRunId, workerId, signal, providerCall }) {
  ...
  if (stage === 'VALIDATION') return executeValidationStage({ client, productionId, stageRunId, workerId });
...
async function verifyVerticalSliceProvenance(client, { productionId, jobId, contextFingerprint }) {
  ...
  const result = await client.query(`SELECT sr.stage, sr.status AS stage_status, sr.output_artifacts, gr.id AS generation_run_id, gr.status AS generation_status, gr.artifact_id, gr.request->'production'->>'contextFingerprint' AS request_context_fingerprint, a.artifact_type, av.output_hash, av.metadata, pb.id AS production_bible_id, pb.context_fingerprint AS bible_context_fingerprint, pb.source_script_artifact_id AS bible_source_script_artifact_id, pb.bible_id FROM v2_1.stage_runs sr LEFT JOIN LATERAL (SELECT gr.* FROM v2_1.generation_runs gr WHERE gr.stage_run_id=sr.id ORDER BY gr.created_at DESC LIMIT 1) gr ON true LEFT JOIN v2_1.artifacts a ON a.id=gr.artifact_id LEFT JOIN v2_1.artifact_versions av ON av.artifact_id=gr.artifact_id LEFT JOIN LATERAL (SELECT pb.* FROM v2_1.production_bibles pb WHERE pb.production_id=$3 AND pb.artifact_id=gr.artifact_id ORDER BY pb.version DESC LIMIT 1) pb ON sr.stage='BIBLE' WHERE sr.job_id=$1 AND sr.stage=ANY($2::text[]) ORDER BY array_position($2::text[], sr.stage), av.version DESC NULLS LAST`, [jobId, stages, productionId]);
  ...
  const idea = rowsByStage.get('IDEA'); const brief = rowsByStage.get('BRIEF'); const concept = rowsByStage.get('CONCEPT'); const script = rowsByStage.get('SCRIPT'); const bible = rowsByStage.get('BIBLE');
  if (idea.artifact_type !== 'IDEA_SET' || brief.artifact_type !== 'CONTENT_BRIEF' || concept.artifact_type !== 'CONCEPT' || script.artifact_type !== 'SCRIPT' || bible.artifact_type !== 'PRODUCTION_BIBLE') throw new Error('Canonical generation artifact vocabulary is invalid');
  ...
  if (!bible.production_bible_id || bible.bible_context_fingerprint !== contextFingerprint || bible.bible_source_script_artifact_id !== script.artifact_id || !bible.bible_id) throw new Error('BIBLE durable provenance is incomplete');
  const deterministicArtifacts = await client.query(`SELECT DISTINCT ON (a.artifact_type) a.artifact_type, a.id AS artifact_id, av.metadata FROM v2_1.artifacts a JOIN v2_1.artifact_versions av ON av.artifact_id=a.id WHERE a.production_id=$1 AND a.artifact_type IN ('SHOTS','ASSET_REQUIREMENTS') ORDER BY a.artifact_type, a.created_at DESC, av.version DESC`, [productionId]);
  const planArtifacts = new Map(deterministicArtifacts.rows.map((row) => [row.artifact_type, row]));
  const shotArtifact = planArtifacts.get('SHOTS'); const assetArtifact = planArtifacts.get('ASSET_REQUIREMENTS');
  if (!shotArtifact || !assetArtifact || shotArtifact.metadata?.stage !== 'SHOT_PLAN' || assetArtifact.metadata?.stage !== 'ASSET_PLAN') throw new Error('Planning artifact provenance is invalid');
...
async function runProductionThroughStages(client, { productionId, jobId, workerId, signal = {}, providerCall = null, leaseSeconds = 120, recover = true, stages, status }) {
  requireClient(client); requireWorker(workerId); if (!productionId || !jobId) throw new Error('productionId and jobId are required');
  const initial = await loadProduction(client, productionId); if (initial.status !== 'RUNNING') throw new Error(`Production is not RUNNING: ${initial.status}`);
  const contextFingerprint = initial.context_fingerprint; if (recover) await recoverExpiredWork(client);
  const claimedJob = await claimJobForProduction(client, { jobId, productionId, workerId, leaseSeconds });
  if (!claimedJob || claimedJob.id !== jobId || claimedJob.production_id !== productionId) throw new Error('Database returned a job outside the requested production boundary');
  const completed = [];
  while (completed.length < stages.length) {
    await assertContextUnchanged(client, productionId, contextFingerprint); await heartbeatJob(client, { jobId, workerId, leaseSeconds });
    ...
    try { await executeClaimedStage(client, { productionId, stage: stage.stage, stageRunId: stage.id, workerId, signal, providerCall }); completed.push(stage.stage); await heartbeatJob(client, { jobId, workerId, leaseSeconds }); }
    catch (error) { await failStage(client, { stageRunId: stage.id, workerId, error: { name: error.name, message: error.message }, retryable: true }); throw error; }
  }
...
async function runProductionThroughBible(client, options = {}) { return runProductionThroughStages(client, { ...options, stages: BIBLE_VERTICAL_SLICE, status: 'BIBLE_COMPLETED' }); }
async function runProductionThroughPlanning(client, options = {}) { const result = await runProductionThroughStages(client, { ...options, stages: FIRST_VERTICAL_SLICE, status: 'PLANNING_COMPLETED' }); const provenance = await verifyVerticalSliceProvenance(client, { productionId: result.productionId, jobId: result.jobId, contextFingerprint: (await loadProduction(client, result.productionId)).context_fingerprint }); return { ...result, provenance }; }
async function runProductionThroughEdit(client, options = {}) { return runProductionThroughStages(client, { ...options, stages: EDIT_VERTICAL_SLICE, status: 'EDIT_COMPLETED' }); }
async function runProductionThroughPlatformAdaptation(client, options = {}) { return runProductionThroughStages(client, { ...options, stages: PLATFORM_ADAPTATION_VERTICAL_SLICE, status: 'PLATFORM_ADAPTATION_COMPLETED' }); }
async function runProductionThroughValidation(client, options = {}) { return runProductionThroughStages(client, { ...options, stages: VALIDATION_VERTICAL_SLICE, status: 'VALIDATION_COMPLETED' }); }

const runProductionThroughScript = runProductionThroughBible;
module.exports = { BIBLE_VERTICAL_SLICE, FIRST_VERTICAL_SLICE, EDIT_VERTICAL_SLICE, PLATFORM_ADAPTATION_VERTICAL_SLICE, VALIDATION_VERTICAL_SLICE, HANDLERS, normalizeSignal, verifyVerticalSliceProvenance, runProductionThroughPlanning, runProductionThroughEdit, runProductionThroughPlatformAdaptation, runProductionThroughValidation, runProductionThroughBible, runProductionThroughScript };
