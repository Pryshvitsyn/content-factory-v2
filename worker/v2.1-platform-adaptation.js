...
async function loadProduction(client, productionId) {
  const result = await client.query(`SELECT p.id, p.status, p.context_fingerprint, p.metadata, cv.target_platform FROM v2_1.productions p JOIN v2_1.content_variants cv ON cv.id=p.content_variant_id WHERE p.id=$1`, [productionId]);
...
async function loadStage(client, productionId, stageRunId, workerId) {
  const result = await client.query(`SELECT sr.id, sr.stage, sr.status, sr.worker_id FROM v2_1.stage_runs sr JOIN v2_1.jobs j ON j.id=sr.job_id WHERE sr.id=$1 AND j.production_id=$2`, [stageRunId, productionId]);
...
async function loadEdit(client, productionId, contextFingerprint) {
  const result = await client.query(`SELECT a.id AS artifact_id, av.output_hash, av.metadata FROM v2_1.artifacts a JOIN v2_1.artifact_versions av ON av.artifact_id=a.id AND av.version=1 WHERE a.production_id=$1 AND a.artifact_type='EDIT' AND a.status='VALID' ORDER BY a.created_at DESC LIMIT 2`, [productionId]);
...
function resolvePlatforms(production) {
  ...
  for (const platform of platforms) if (!EDITION_PLATFORMS.includes(platform)) throw new Error(`Unsupported target platform: ${platform}`);
  return platforms;
}
...
function buildPlatformManifest({ production, edit, platforms }) {
  ...
  const editions = platforms.map((platform) => ({
    ...
    timeline: timeline.map((item) => ({
      ...
      adaptation: { captions: platform === 'YOUTUBE' ? 'OPTIONAL' : 'DOWNSTREAM_REQUIRED', framing: 'SOURCE_PRESERVED', audio: 'SOURCE_PRESERVED' },
    })),
    renderProfile: production.metadata?.platformProfiles?.[platform] || {},
  }));
  const manifest = { type: 'EDITIONS', version: 1, contextFingerprint: production.context_fingerprint, editFingerprint: edit.output_hash, sourceEditArtifactId: edit.artifact_id, durationMs, editions };
...
async function executePlatformAdaptationStage({ client, productionId, stageRunId, workerId } = {}) {
  ...
  try {
    ...
    const existing = await client.query(`SELECT a.id AS artifact_id, av.output_hash, av.metadata FROM v2_1.artifacts a JOIN v2_1.artifact_versions av ON av.artifact_id=a.id AND av.version=1 WHERE a.production_id=$1 AND a.artifact_type='EDITIONS' AND a.status='VALID' ORDER BY a.created_at DESC LIMIT 1`, [productionId]);
    if (existing.rows.length) {
      const row = existing.rows[0];
      if (row.output_hash !== outputHash || row.metadata?.contextFingerprint !== production.context_fingerprint || row.metadata?.editArtifactId !== edit.artifact_id || row.metadata?.editFingerprint !== edit.output_hash) throw new Error('Existing canonical EDITIONS artifact conflicts with requested EDIT provenance');
    ...
    await client.query(`INSERT INTO v2_1.artifact_versions(artifact_id,version,input_hash,output_hash,metadata) VALUES($1,1,$2,$3,$4::jsonb)`, [artifactId, edit.output_hash, outputHash, JSON.stringify({ stage: 'PLATFORM_ADAPTATION', contextFingerprint: production.context_fingerprint, editArtifactId: edit.artifact_id, editFingerprint: edit.output_hash, platforms, durationMs, manifest })]);
    for (const edition of manifest.editions) {
