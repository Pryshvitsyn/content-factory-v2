...
class AvatarMotionPilotService {
  ...
  selection(route=ROUTE) { try{return this.providerCatalog.resolveSelection(route);}catch(error){if(error.code==='CREDENTIALS_MISSING') return Object.freeze({provider:route.provider,model:route.model,profile:route.profile||'STANDARD',capability:route.capability,adapterFamily:route.model==='wan-video/wan-2.7-r2v'?'replicate-wan-2.7-r2v':'replicate-wan-3'});throw new AvatarStudioError(error.status||409,error.code||'MOTION_PILOT_PROVIDER_UNAVAILABLE',error.message,error.details);} }
  async identityReferenceBundle(c, brandId) {
  ...
  async canonicalRequest({c,plan}) { const bundle=await this.identityReferenceBundle(c,plan.brandId); if(bundle.bundleFingerprint!==plan.identityReferenceBundle?.bundleFingerprint) throw new AvatarStudioError(409,'STALE_APPROVAL','Identity reference bundle changed; create a new plan and approval'); const referenceImages=[]; for(const reference of bundle.references) { const bytes=await this.storage.get({key:reference.storageKey}); if(!Buffer.isBuffer(bytes)||!bytes.length||crypto.createHash('sha256').update(bytes).digest('hex')!==reference.contentHash) throw new AvatarStudioError(409,'IDENTITY_REFERENCE_HASH_MISMATCH','Identity reference bytes changed'); referenceImages.push(`data:${reference.mimeType};base64,${bytes.toString('base64')}`); } const request={capability:'REFERENCE_TO_VIDEO',providerSelection:{provider:ROUTE.provider,model:ROUTE.model},providerPrompt:plan.prompt,negativePrompt:plan.negativeGuidance.join(', '),resolution:ROUTE.resolution,aspectRatio:ROUTE.aspectRatio,durationSeconds:ROUTE.durationSeconds,shotType:'single',references:{referenceImages,referenceVideos:[]},identityReferenceBundle:bundle}; return Object.freeze({request,evidence:bundle}); }
  ...
  async recoverExisting({executionId,attemptId,...scope}={}) {
    ...
    const historicalRoute={provider:execution.provider,model:execution.model,profile:execution.preflightSnapshot?.profile||'STANDARD',capability:execution.capability};
  ...
  async reviewIdentity({executionId,attemptId,decision,reasonCode,humanNote=null,...scope}={}) {
    ...
    if(normalized==='FAIL'&&!String(reasonCode||'').trim()) throw new AvatarStudioError(400,'MOTION_PILOT_IDENTITY_REASON_REQUIRED','A failure reason is required');
