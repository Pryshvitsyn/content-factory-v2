...
const { FfmpegReferenceGeometryNormalizer, compatible, referenceEvidence } = require('../v2.10.2/reference-geometry');
const { assertWan3ReferenceGeometry } = require('../providers/replicate-universal-video-adapter');
const crypto = require('node:crypto');

const ROUTE = Object.freeze({ provider:'replicate', model:'alibaba/wan-3', profile:'STANDARD', capability:'IMAGE_TO_VIDEO',
  resolution:'720p', aspectRatio:'9:16', durationSeconds:5, plannedCalls:1, audioRequested:false });
const NEGATIVE = Object.freeze(['large head turns','face morphing','age drift','hairstyle drift','wardrobe changes','body proportion changes',
  'extra people','hands entering frame','camera movement','zoom','dramatic expressions','talking','mouth articulation','dialogue','audio']);
...
function motionPrompt(minor) { return [
  'TECHNICAL AVATAR IDENTITY AND MOTION PILOT ONLY. Not an advertisement or public content.',
  'One certified person, chest-up, static camera, neutral simple background, one continuous 5 second vertical 9:16 shot.',
  'Preserve the certified identity, apparent age, hairstyle, proportions, and certified neutral clothing exactly.',
  'Natural blink, tiny natural head movement, very subtle breathing/body micro-motion, and at most a tiny expression shift. Hands stay outside frame. Mouth remains naturally closed and neutral. No speech, lip sync, dialogue, voice cloning, or audio.',
  minor ? 'MINOR SAFETY: family-safe and age-appropriate. Preserve apparent age and certified neutral clothing. No adultization, endorsement, testimonial framing, or adult styling.' : null,
  `Suppress: ${NEGATIVE.join(', ')}.`
].filter(Boolean).join('\n'); }
...
class AvatarMotionPilotService {
  constructor({repository,providerCatalog,providerGateway=null,assetIntakeService=null,storage=null,mediaInspector=null,geometryNormalizer=null,adapterFactory=createVideoAdapter,env=process.env,actor='local-operator'}={}) { if(!repository||!providerCatalog) throw new Error('AvatarMotionPilotService requires repository and providerCatalog'); Object.assign(this,{repository,providerCatalog,providerGateway,assetIntakeService,storage,mediaInspector,geometryNormalizer:geometryNormalizer||new FfmpegReferenceGeometryNormalizer({inspector:mediaInspector||undefined}),adapterFactory,env,actor}); }
  async context(scope={}) {
  ...
  async canonicalRequest({c,plan}) { const original=await this.storage.get({key:c.intake.artifactStorageKey}); if(!Buffer.isBuffer(original)||crypto.createHash('sha256').update(original).digest('hex')!==plan.referenceContentHash) throw new AvatarStudioError(409,'CERTIFIED_REFERENCE_HASH_MISMATCH','Certified chest-up reference bytes changed'); const normalized=await this.geometryNormalizer.normalize({bytes:original,contentType:c.intake.mimeType,expectedAspectRatio:'9:16',resolution:'720p'}); if(!compatible(normalized.after,'9:16')) throw new AvatarStudioError(409,'REFERENCE_GEOMETRY_MISMATCH','Normalized reference is not canonical 9:16'); const evidence=referenceEvidence({result:normalized,expectedAspectRatio:'9:16',source:{artifactId:c.intake.artifactId,version:c.intake.artifactVersion,contentHash:plan.referenceContentHash},referenceBytes:normalized.bytes}); const request={capability:'IMAGE_TO_VIDEO',providerSelection:{provider:ROUTE.provider,model:ROUTE.model},providerPrompt:plan.prompt,negativePrompt:plan.negativeGuidance.join(', '),resolution:ROUTE.resolution,aspectRatio:ROUTE.aspectRatio,durationSeconds:ROUTE.durationSeconds,audio:{requested:false},references:{firstFrame:`data:${normalized.contentType};base64,${normalized.bytes.toString('base64')}`},referenceGeometry:evidence}; assertWan3ReferenceGeometry(request,request.references,'9:16'); return Object.freeze({request,evidence,normalized}); }
  ...
  async generate({executionId,...scope}={}) {
    ...
    try {
      ...
      const result=await adapter.generate({capability:'IMAGE_TO_VIDEO',canonicalRequest:prepared.request,idempotencyKey:attempt.idempotencyKey,onProviderRequest:async({requestId,status})=>this.repository.recordMotionPilotProviderRequest({attempt,requestId,status,actor:this.actor})});
  ...
  async recoverExisting({executionId,attemptId,...scope}={}) {
    ...
    if(attempt.status!=='FAILED'||!attempt.providerRequestId||execution.provider!=='replicate'||execution.model!==ROUTE.model||execution.capability!=='IMAGE_TO_VIDEO') throw new AvatarStudioError(409,'MOTION_PILOT_RECOVERY_INELIGIBLE','Only the persisted failed Replicate Wan IMAGE_TO_VIDEO request can be recovered');
