...
class AvatarMotionPilotService {
  ...
  selection(route=ROUTE) { try{return this.providerCatalog.resolveSelection(route);}catch(error){throw new AvatarStudioError(error.status||409,error.code||'MOTION_PILOT_PROVIDER_UNAVAILABLE',error.message,error.details);} }
  async identityReferenceBundle(c, brandId) {
    const reference=async ({role,viewpoint,intake})=>{const bytes=await this.storage.get({key:intake.artifactStorageKey});if(!Buffer.isBuffer(bytes)||!bytes.length) throw new AvatarStudioError(409,'IDENTITY_REFERENCE_BYTES_MISSING','Identity reference storage bytes are missing');const contentHash=crypto.createHash('sha256').update(bytes).digest('hex');if(contentHash!==intake.contentHash) throw new AvatarStudioError(409,'IDENTITY_REFERENCE_HASH_MISMATCH','Identity reference bytes changed');return {role,viewpoint,intakeId:intake.id,artifactId:intake.artifactId,artifactVersion:intake.artifactVersion,contentHash,mimeType:intake.mimeType,byteSize:bytes.length,storageKey:intake.artifactStorageKey};};
