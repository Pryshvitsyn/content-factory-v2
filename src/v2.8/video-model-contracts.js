...
function registerVideoModelContract(contract){
  ...
  const frozen=Object.freeze({...contract,providerFields:immutable(contract.providerFields||[]),inputModes:immutable(contract.inputModes),capabilities:immutable(contract.capabilities),parameters:immutable(contract.parameters),limits:immutable(contract.limits),provenance:immutable(contract.provenance),output:immutable(contract.output||{}),pricing:immutable(contract.pricing||{}),technicalQa:immutable(contract.technicalQa),workflowCompatibility:immutable(contract.workflowCompatibility)});
...
function compareVideoModelSchema({provider,model,schema}={}){const contract=getVideoModelContract(provider,model);if(!contract)fail('VIDEO_MODEL_CONTRACT_NOT_FOUND',`No reviewed video model contract for ${provider}/${model}`);const actual=Object.keys(schema?.properties||schema?.input?.properties||{}).sort(),reviewed=[...(contract.providerFields||[])].sort(),added=actual.filter(x=>!reviewed.includes(x)),removed=reviewed.filter(x=>!actual.includes(x));return Object.freeze({status:added.length||removed.length?'SCHEMA_DRIFT':'CURRENT',provider,model,contractVersion:contract.contractVersion,added,removed});}
...
function resolveVideoModelRequest({provider,model,request={}}={}){
  ...
  return Object.freeze({...snapshot,requestFingerprint:fingerprint(snapshot)});
}
