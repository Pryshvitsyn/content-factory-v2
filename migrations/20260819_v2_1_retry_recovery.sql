...
BEGIN
  ...
  LOOP
    IF stage_row.attempt < stage_row.max_attempts THEN
      ...
      UPDATE v2_1.stage_runs
         SET status='FAILED',
             error=jsonb_build_object('code','LEASE_EXPIRED','recovered_at',now()),
             worker_id=NULL,
      ...
      INSERT INTO v2_1.stage_runs(
        ...
        stage_row.job_id, stage_row.stage, next_attempt, 'RETRYING',
        stage_row.input_artifacts, stage_row.input_fingerprint, stage_row.max_attempts,
    ...
    ELSE
      UPDATE v2_1.stage_runs
         SET status='DEAD_LETTER',
             error=jsonb_build_object('code','LEASE_EXPIRED_MAX_ATTEMPTS','attempt',stage_row.attempt),
             worker_id=NULL,
             lease_expires_at=NULL,
             heartbeat_at=now(),
             completed_at=now(),
      ...
      UPDATE v2_1.jobs
         SET status='FAILED',
             error=jsonb_build_object('code','STAGE_LEASE_EXPIRED','stage',stage_row.stage),
             last_error=jsonb_build_object('code','STAGE_LEASE_EXPIRED','stage',stage_row.stage),
             worker_id=NULL,
             lease_expires_at=NULL,
             heartbeat_at=now(),
             completed_at=now(),
  ...
  FOR job_row IN
    ...
    WHERE status='RUNNING'
      ...
      AND lease_expires_at < now()
      AND NOT EXISTS (
        SELECT 1 FROM v2_1.stage_runs s
        WHERE s.job_id=v2_1.jobs.id AND s.status='RUNNING'
      )
    FOR UPDATE SKIP LOCKED
  LOOP
    IF job_row.attempt < job_row.max_attempts THEN
      UPDATE v2_1.jobs
         SET status='RETRYING',
             ...
             lease_expires_at=NULL,
             heartbeat_at=now(),
             last_error=jsonb_build_object('code','JOB_LEASE_EXPIRED','previous_attempt',job_row.attempt),
             updated_at=now()
       WHERE id=job_row.id;
    ...
    ELSE
      UPDATE v2_1.jobs
         SET status='DEAD_LETTER',
             error=jsonb_build_object('code','JOB_LEASE_EXPIRED_MAX_ATTEMPTS','attempt',job_row.attempt),
             last_error=jsonb_build_object('code','JOB_LEASE_EXPIRED_MAX_ATTEMPTS'),
             worker_id=NULL,
             lease_expires_at=NULL,
             heartbeat_at=now(),
             completed_at=now(),
