...
-- Backfill the canonical version record from the legacy artifact row.
...
SET content_json = a.content_json,
    ...
    metadata = a.metadata,
    content_size = CASE
      WHEN a.metadata ? 'size' THEN NULLIF(a.metadata->>'size', '')::bigint
      ELSE NULL
    END,
...
-- Existing artifact rows are compatibility projections. Their version must
-- always represent the newest canonical artifact_version, never a second
-- version history.
...
-- Mark the legacy column explicitly so future code does not treat it as an
-- independent version source. PostgreSQL comments are harmless to execution.
