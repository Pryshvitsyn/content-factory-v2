-- V2.1 execution recovery retry fix
-- Historical FAILED stage attempts must not poison a job when a newer RETRYING/RUNNING/COMPLETED attempt exists.
-- A stage is terminally failed only when its latest attempt is FAILED and no newer attempt can continue it.
    ...
    UPDATE v2_1.jobs j
       SET status = CASE WHEN j.attempts < j.max_attempts THEN 'RETRYING' ELSE 'FAILED' END,
           next_attempt_at = CASE
             ...
             THEN now() + make_interval(secs => LEAST(300, power(2, LEAST(j.attempts, 8))::integer))
...
COMMENT ON FUNCTION v2_1.recover_expired_work() IS
  'Recovers expired leases without treating historical failed stage attempts as terminal while a newer retry attempt exists; terminal failure is based on the latest stage attempt.';
